...
export const buildHandler: (
  ...
  if (actorId) {
    // The class `AccessToken` has a list of predefined claims,
    // and the method `save` will only accept the predefined claims when generating the JWT.
    // So we have to use `extra` to store the `act` claim, and then use `extraTokenClaims` to
    // extract it when generating the JWT.
